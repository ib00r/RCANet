...
import os
import datetime
from nets.scSE_Unet import SCSEUnet
import numpy as np
import torch
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    # model_path  = "model_data/unet_vgg_voc.pth"#预训练模型
    model_path  = ""#不加载骨干网络
    # model_path  = "model_data/unet_resnet_voc.pth"#预训练模型
    # model_path = "logs/ep025-loss0.011-val_loss0.012.pth"
    #-----------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    num_workers     = 8
    #在dell 服务器上用cpu_count() ====> 32多线程会卡死 训练没有任何进展
    ...
    model = SCSEUnet(in_channel=3, out_channel=num_classes).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
        ...
        #---------------------------------------------------
        # 加入batch_norm层 需要改加载方式
        #----------------------------------------------
        for k, v in pretrained_dict.items():
            if model_path  == "model_data/vgg16_bn-6c64b313.pth":
                k = 'vgg.' + k
            if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                temp_dict[k] = v
                load_key.append(k)
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                ###################################################################################
                #  会将当前的batch传入unet_dataset_collate 进行数据处理（匹配图像和标签，增强，调整尺寸）
                ###################################################################################
                gen             = DataLoader(train_dataset, shuffle = shuffle, batch_size = batch_size, num_workers = num_workers, pin_memory=True,
                                            drop_last = True, collate_fn = unet_dataset_collate, sampler=train_sampler)
                gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = batch_size, num_workers = num_workers, pin_memory=True, 
